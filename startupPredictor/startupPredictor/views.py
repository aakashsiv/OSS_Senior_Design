...
def results_view(request):
    ...
    result = float(amount_to_invest) * float(organization.last_equity_funding_amount_currency_in_usd_field)
    
    # Pass the website to the results page
    context = {'startup_name': startup_name, 'result': result}
...
def top_picks(request):
    organizations = Organizations.objects.exclude(last_equity_funding_amount_currency_in_usd_field__isnull=True).order_by('-last_equity_funding_amount_currency_in_usd_field')[:10]
